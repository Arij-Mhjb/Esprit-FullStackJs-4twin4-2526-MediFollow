...
function encryptPrivateKey(plaintext) {
  if (!process.env.WALLET_ENCRYPTION_KEY) {
    throw new Error("WALLET_ENCRYPTION_KEY is not defined in .env");
  }
...
async function main() {
  const email = "walaeddine1207@gmail.com";
  const password = "Wallou12@";
  const firstName = "Walaeddine";
  const lastName = "Riahii";

  console.log(`🚀 Création du compte docteur pour: ${email}...`);
  ...
  await prisma.doctorProfile.create({
    data: {
      ...
      bio: "Médecin spécialiste en cardiologie.",
    },
  });
