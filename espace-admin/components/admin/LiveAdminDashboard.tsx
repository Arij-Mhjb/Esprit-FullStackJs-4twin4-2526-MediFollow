  ...
  return (
    <div
      className={`glass-panel glass-scan group rounded-[2rem] border border-cyan-300/20 p-7 shadow-[0_20px_60px_rgba(2,6,23,0.45)] transition-all duration-500 [transform-style:preserve-3d] hover:[transform:rotateX(6deg)_rotateY(-6deg)_translateY(-6px)] ${
        flash
          ? "glass-neon scale-[1.04]"
          : ""
      }`}
    >
      <div className={`inline-block rounded-2xl p-3 mb-6 ${colors[color]} [transform:translateZ(20px)]`}>
        {icon}
      </div>
...
export default function LiveAdminDashboard({
  ...
  const quickActions = [
    { href: "/dashboard/admin/alerts", label: "Review incidents", desc: "Prioritize open and critical alerts" },
    { href: "/dashboard/admin/users", label: "Manage users", desc: "Roles, access, and account status" },
    { href: "/dashboard/admin/analytics", label: "Open analytics", desc: "Trends and care performance" },
    { href: "/dashboard/admin/audit", label: "Audit logs", desc: "Track admin operations and events" },
  ];

  const openRate = stats.totalAlerts > 0 ? Math.round((stats.openAlerts / stats.totalAlerts) * 100) : 0;
  const criticalRate = stats.totalAlerts > 0 ? Math.round((stats.criticalAlerts / stats.totalAlerts) * 100) : 0;
  ...
  return (
    <div className="relative">
      ...
      <section className="glass-panel glass-neon glass-scan relative mb-8 overflow-hidden rounded-[2rem] border border-cyan-300/20 p-6 shadow-[0_30px_90px_rgba(2,6,23,0.5)] lg:p-8">
        ...
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <p className="text-[11px] font-black uppercase tracking-[0.22em] text-cyan-600 dark:text-cyan-200/80">MediFollow Admin Workspace</p>
            ...
            <p className="mt-3 text-sm leading-relaxed text-slate-600 dark:text-cyan-100/80 lg:text-base">
              Monitor incidents in real time, keep user governance under control, and coordinate clinical teams with fewer clicks.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:min-w-[520px]">
            <div className="glass-panel rounded-xl border border-cyan-300/20 p-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-cyan-100/70">Open</p>
              <p className="mt-1 text-2xl font-black text-rose-600">{stats.openAlerts}</p>
            </div>
            <div className="glass-panel rounded-xl border border-cyan-300/20 p-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-cyan-100/70">Critical</p>
              <p className="mt-1 text-2xl font-black text-amber-600">{stats.criticalAlerts}</p>
            </div>
            <div className="glass-panel rounded-xl border border-cyan-300/20 p-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-cyan-100/70">Resolved</p>
              <p className="mt-1 text-2xl font-black text-emerald-600">{stats.resolvedAlerts}</p>
            </div>
            <div className="glass-panel rounded-xl border border-cyan-300/20 p-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-cyan-100/70">Users</p>
              <p className="mt-1 text-2xl font-black text-indigo-600">{stats.totalUsers}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mb-8 grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div className="glass-panel rounded-[2rem] border border-cyan-300/20 p-6 shadow-[0_24px_70px_rgba(2,6,23,0.42)] lg:p-8 xl:col-span-2">
          <div className="mb-6 flex items-center justify-between">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">Incident Operations</h3>
            <Link href="/dashboard/admin/alerts" className="glass-panel inline-flex items-center gap-2 rounded-xl border border-slate-200 dark:border-cyan-300/25 px-3 py-2 text-xs font-bold uppercase tracking-wide text-slate-600 dark:text-cyan-100 transition hover:border-slate-400 dark:hover:border-cyan-300 hover:text-slate-900 dark:hover:text-white">
              Open incidents
              <ArrowRight size={14} />
            </Link>
          </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-900/45">
                <div className="h-full rounded-full bg-emerald-500 transition-all duration-700" style={{ width: `${resolvedRate}%` }} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-900/45">
                <div className="h-full rounded-full bg-rose-500 transition-all duration-700" style={{ width: `${openRate}%` }} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-900/45">
                <div className="h-full rounded-full bg-amber-500 transition-all duration-700" style={{ width: `${criticalRate}%` }} />
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2">
            {quickActions.slice(0, 2).map((action) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-slate-500 dark:text-cyan-100/70">{action.desc}</p>
              </Link>
            ))}
          </div>
        </div>
          ...
          <div className="glass-panel glass-scan rounded-2xl border border-cyan-300/20 p-4">
            <div className="mb-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white">Mission Control</h3>
                  <p className="text-[11px] uppercase tracking-widest text-slate-400 dark:text-cyan-100/65">Operational copilot</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 rounded-2xl bg-slate-50 dark:bg-slate-900/35 p-4 min-h-[110px] border border-slate-200 dark:border-cyan-300/10">
              {aiLoading && <p className="text-sm text-slate-500 dark:text-cyan-100/75">Analyzing your request...</p>}
              {!aiLoading && aiError && <p className="text-sm text-rose-600">{aiError}</p>}
              {!aiLoading && !aiError && !aiResult && (
                <p className="text-sm text-slate-500 dark:text-cyan-100/75">Run a quick prompt to get an AI operations briefing.</p>
              )}
              {!aiLoading && aiResult && (
                <div className="space-y-2">
                  <p className="text-sm font-medium text-slate-800 dark:text-slate-100">{aiResult.answer}</p>
                  {Array.isArray(aiResult.suggestions) && aiResult.suggestions.length > 0 && (
                    <ul className="space-y-1">
                      {aiResult.suggestions.slice(0, 3).map((item, idx) => (
                        <li key={`${item}-${idx}`} className="text-xs text-slate-500 dark:text-cyan-100/70">
                          - {item}
                        </li>
                      ))}
                    </ul>
            ...
            {quickActions.slice(2).map((action) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-100">{action.label}</p>
                  <ArrowRight size={14} className="text-slate-400 dark:text-cyan-100/60" />
                </div>
                <p className="mt-1 text-xs text-slate-500 dark:text-cyan-100/70">{action.desc}</p>
              </Link>
            ))}
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div className="glass-panel rounded-[2rem] border border-cyan-300/20 p-6 shadow-[0_24px_70px_rgba(2,6,23,0.42)] lg:p-8">
          <h3 className="mb-6 text-xl font-bold text-slate-900 dark:text-white">System Health</h3>
            <div className="space-y-5">
              <StatusRow
