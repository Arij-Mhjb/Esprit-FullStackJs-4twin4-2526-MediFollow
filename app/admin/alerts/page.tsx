...
export default function AdminAlertsPage() {
  ...
  useEffect(() => {
    const pusher = new Pusher(process.env.NEXT_PUBLIC_PUSHER_KEY || "ba707a9085e391ba151b", {
      cluster: "eu",
    });
  ...
  async function loadAlerts() {
    try {
      ...
      if (result.success && result.alerts) {
        ...
        setFilteredAlerts([]);
        setErrorMessage(
          result?.detail || result?.error || "Failed to load alerts"
        );
      }
    } catch (error) {
      ...
      <div>
        ...
        <div className="mb-6 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {statusFilter !== "ALL" && (
              <span
                className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${getStatusColor(statusFilter)}`}
              >
                {getStatusIcon(statusFilter)} Status:{" "}
                {getStatusLabel(statusFilter)}
              </span>
            )}
            {severityFilter !== "ALL" && (
              <span
                className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${getSeverityColor(severityFilter)}`}
              >
                {getSeverityIcon(severityFilter)} Severity:{" "}
        ...
        {filteredAlerts.length === 0 ? (
          <div className="rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-12 text-center">
            <AlertCircle
              size={48}
              className="mx-auto text-gray-300 dark:text-slate-700 mb-3"
            />
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${getSeverityColor(alert.severity)}`}
                        >
                          {getSeverityIcon(alert.severity)}{" "}
                          {getSeverityLabel(alert.severity)}
                        </span>
                        <span
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${getStatusColor(alert.status)}`}
                        >
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 dark:text-slate-400">
                        {alert.patient?.user && (
                          <span className="flex items-center gap-1">
                            <User size={14} /> {alert.patient.user.firstName}{" "}
                    ...
                    <ChevronRight
                      size={18}
                      className="text-gray-400 dark:text-slate-600 group-hover:text-gray-600 dark:group-hover:text-slate-400 transition-colors ml-4"
                    />
                  </div>
                </div>
              </Link>
